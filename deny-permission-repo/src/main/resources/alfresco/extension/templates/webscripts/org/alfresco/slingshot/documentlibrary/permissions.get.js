...
/**
 * Main entry point: Retrieve permissions and associated metadata for given node taking into account also denied permissions
 *
...
function getPermissions()
{
   ...
   {
        logger.log("Overwriting permissions.get.js");
   }
   /**
   ...
   var isInherited = node.inheritsPermissions(),
      nodePermissions = parsePermissions(enhancedPermissions.getDirectPermissions(node.nodeRef), settable),
      inheritedPermissions = [],
      canReadInherited = true;

   if (node.parent.hasPermission("ReadPermissions"))
   {
      inheritedPermissions = parsePermissions(enhancedPermissions.getPermissions(node.parent.nodeRef), settable);
   }
   else
   {
...
function parsePermissions(p_permissions, p_settable)
{
   ...
   for (i = 0, ii = p_permissions.length; i < ii; i++)
   {
      ...
      //Removed to show all permissions ALLOWED & DENIED
      //if (tokens[0] == "ALLOWED")
      //{
